...
def parseArgs():
    if (len(sys.argv) != 9):
        ...
        main(sys.argv[2], sys.argv[4], sys.argv[6], sys.argv[-1])
...
def main(eccPublicKeyPath, data, signature, requester):
    pemPublicKey = utils.readFile(eccPublicKeyPath)
    requesterFileContents = open(requester, 'r').read().split('%S%') 
    blindComponents, pRComponents = requesterFileContents[0], requesterFileContents[1]
