...
def main(eccPrivateKeyPath, bmsg):
    initComponents = open('init.txt', 'r').read().split('%S%')[0]
    pemKey = utils.readFile(eccPrivateKeyPath)
    print("Input")
    passphrase = raw_input("Passphrase: ")
    errorCode, blindSignature = eccblind.generateBlindSignature(pemKey, passphrase, bmsg, initComponents)
